...
export default function Navbar() {
  ...
  const handleNavClick = (e, href) => {
    // Only handle anchor links, let normal navigation work otherwise
    if (href.startsWith('#')) {
      e.preventDefault();
          ...
          {NAV_LINKS.map((link) => (
            <Link
              ...
              className={`group relative flex items-center justify-center w-10 h-10 rounded-full hover:bg-light-100 dark:hover:bg-dark-800 transition-colors ${
                activeSection === link.href.replace('#', '') ? 'bg-light-100 dark:bg-dark-800' : ''
              }`}
            >
              <span className={`${
            ...
            <nav className="container mx-auto px-4 py-4">
              {NAV_LINKS.map((link) => (
                <Link
                  ...
                  className={`flex items-center space-x-3 p-3 hover:bg-light-100 dark:hover:bg-dark-800 rounded-lg transition-colors ${
                    activeSection === link.href.replace('#', '') 
                      ? 'bg-light-100/50 dark:bg-dark-800/50 text-primary-600 dark:text-primary-400' 
                      : 'text-dark-700 dark:text-light-200'
                  }`}
                >
                  {renderIcon(link.icon)}
